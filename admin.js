(() => {
  ...
  const renderMetrics = (orders, events) => {
    ...
    const unpaidOrdersToday = todayOrders.filter((order) => order.status === 'pending');
    const unpaidToday = unpaidOrdersToday.length;
    const unpaidValueToday = unpaidOrdersToday.reduce((sum, order) => sum + Number(order.amount || 0), 0);
    ...
    values[3].textContent = money(unpaidValueToday);
    document.querySelector('[data-pending-count]').textContent = `${unpaidToday} aguardando pagamento`;
    document.querySelector('[data-unpaid-today-count]').textContent = `${unpaidToday} pedido${unpaidToday === 1 ? '' : 's'} aguardando pagamento hoje`;
